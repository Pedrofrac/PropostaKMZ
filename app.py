...
# --- FUNÇÃO CENTRAL DE FILTRAGEM (MATCHING INTELIGENTE) ---
def filtra_propostas(criterios, lista_de_propostas):
    resultados = []
    
    # Normalização dos critérios numéricos da PESQUISA
    try:
        ...
        # Se houver erro na conversão dos critérios, retorna lista vazia por segurança
        return []

    for proposta in lista_de_propostas:
        compativel = True
        
        # 1. Filtros de Texto (Categoria, Fabricante, Modelo, Cor)
        # Lógica: Se a proposta no banco tem o campo VAZIO, ela aceita TUDO (Curinga).
        # Se a pesquisa tem o campo VAZIO, ela busca TUDO.
        # O conflito só existe se AMBOS tiverem valores e eles forem DIFERENTES.
        for campo in ['categoria', 'fabricante', 'modelo', 'cor']:
            valor_proposta = proposta.get(campo, "")
            valor_criterio = criterios.get(campo, "")
            
            if valor_proposta and valor_criterio and valor_proposta != valor_criterio:
                compativel = False
                break
        if not compativel: continue

        # 2. Filtro de Câmbio (Exclusão Mútua)
        manual_desejado = criterios.get('cambio_manual', False)
        automatico_desejado = criterios.get('cambio_automatico', False)
        
        # Se a proposta define um câmbio, verificamos se bate com a pesquisa.
        # Se a proposta não define câmbio (""), ela aparece para qualquer filtro.
        if proposta.get('cambio'):
        ...
        # 3. Filtros de Intervalo (Ano e Preço)
        try:
            # Normalização dos valores da PROPOSTA (BD)
            # Se vazio no BD: Min vira 0, Max vira Infinito (aceita qualquer faixa)
            prop_ano_min = int(proposta.get('ano_min') or 0)
            prop_ano_max = int(proposta.get('ano_max') or 9999)
            
            # Verifica intersecção de intervalos de Ano
            # (O máximo da proposta deve ser maior que o mínimo da busca E o mínimo da proposta menor que o máximo da busca)
            if not (prop_ano_max >= crit_ano_min and prop_ano_min <= crit_ano_max):
                compativel = False

            if compativel: # Só checa preço se ano passou
                ...
                # Verifica intersecção de intervalos de Preço
    ...
    if proposta_id:
        ...
        for i, p in enumerate(todas_as_propostas):
            if str(p.get('id')) == str(proposta_id):
                proposta_data['data_inclusao'] = p.get('data_inclusao')
